...
public class OurServicesReportPanel extends javax.swing.JPanel {
    ...
    public void loadServices1() {

        try {
            String query = "SELECT services.id, services.name AS service_name, "
                    + "vehicle_type.name AS vehicle_type_name, services.charge "
                    + "FROM `services` "
                    + "INNER JOIN `vehicle_type` ON `services`.`vehicle_type_id` = `vehicle_type`.`id` ";

            String whereClause = "";
            double minCharge = PriceFrom1.getText().isEmpty() ? 0 : Double.parseDouble(PriceFrom1.getText());
            double maxCharge = PriceTo1.getText().isEmpty() ? 0 : Double.parseDouble(PriceTo1.getText());

            if (minCharge > 0 && maxCharge == 0) {
                whereClause += "`services`.`charge` >= " + minCharge + " ";
            } else if (minCharge == 0 && maxCharge > 0) {
                whereClause += "`services`.`charge` <= " + maxCharge + " ";
            } else if (minCharge > 0 && maxCharge > 0) {
                whereClause += "`services`.`charge` >= " + minCharge + " AND `services`.`charge` <= " + maxCharge + " ";
            }
            ...
            ResultSet resultSet = new ServicesController().customQuery(query);

            DefaultTableModel dtm = (DefaultTableModel) ourServicesViewTable2.getModel();
            dtm.setRowCount(0);
            ...
            while (resultSet.next()) {
                ...
                Vector<String> vector = new Vector<>();
                vector.add(resultSet.getString("id"));
                vector.add(resultSet.getString("service_name"));
                vector.add(resultSet.getString("vehicle_type_name"));
                vector.add(resultSet.getString("charge"));
                dtm.addRow(vector);
            }
        } catch (Exception e) {
            e.printStackTrace();
            logger.warning("Error while filtering services by charge: " + e.getMessage());
        }
